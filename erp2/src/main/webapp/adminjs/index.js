...
function serverLogin() {
    ...
    $.ajax({
    	...
    	success:function(data){
			...
			if(!data.success) {
				flag = false;
				$.messager.alert('修改密码失败','原始密码输入错误','info');
			}
    	}
    })
    
    if(flag){
    	$.post('emp_editPwd',{id:loginUser.id,pwd:$newpass.val()},function(msg) {
...
$(function() {
	...
	$('#loginOut').click(function(){
		$.messager.confirm('确认对话框', '您确认要退出吗？', function(r){
			if (r){
				
				$.post('emp_loginout',function(data){
					//跳转到登录界面
					window.location.href = 'login.html';
				})
