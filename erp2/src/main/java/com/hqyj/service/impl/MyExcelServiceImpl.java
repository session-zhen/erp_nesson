...
public class MyExcelServiceImpl implements MyExcelService {
	@Override
	public void myExcelOut(Orders orders) {
		...
		if(null != orders.getCreater()){//下单员
			sheet.getRow(3).getCell(3).setCellValue(orders.getEnderName());
		}
		if(null != orders.getChecker()){//审核员
			sheet.getRow(4).getCell(3).setCellValue(orders.getStarterName());
		}
		if(null != orders.getStarter()){//采购员
			sheet.getRow(5).getCell(3).setCellValue(orders.getCreaterName());
		}
		...
		//保存工作簿到本地目录
		try {
			book.write(new FileOutputStream("d:\\订单.xls"));
			book.close();
